...
from api.data import datasets, features, normalization, splitters, targets
...
def test_split_by_date_no_overlap(sample_daily_df, phase0_cfg):
    ...
    assert (
        ...
        < pd.to_datetime(val['timestamp']).min()
    )
    assert (
        pd.to_datetime(val['timestamp']).max() < pd.to_datetime(test['timestamp']).min()
    )
...
def test_dataset_shapes_match_config_seq_len(sample_daily_df, phase0_cfg):
    ...
    df = targets.add_return_targets(df, [1])
    df = df.dropna()
    feature_cols = phase0_cfg.get('data', {}).get(
        'feature_cols',
        ['close', 'volume', 'sma_15', 'sma_45', 'rsi_14', 'macd', 'macd_signal', 'obv'],
    )
