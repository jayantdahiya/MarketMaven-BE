...
def test_forecast_daily_invalid_asset_422(client):
    r = client.post("/forecast/daily", json={"asset_id": "invalid ticker!", "model": "lstm_baseline"})
    assert r.status_code == 422


def test_forecast_daily_invalid_model_400(client):
    r = client.post("/forecast/daily", json={"asset_id": "AAPL", "model": "unknown_model"})
    assert r.status_code == 400


def test_forecast_daily_no_checkpoint_503(client):
    """When no checkpoint exists, LSTM forecast returns 503."""
    r = client.post("/forecast/daily", json={"asset_id": "AAPL", "model": "lstm_baseline"})
