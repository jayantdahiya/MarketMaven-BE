    ...
    def signup(self, email: str, password: str) -> dict:
        try:
            ...
            return {
                ...
                else str(getattr(user.get('user'), 'id', '')),
                'access_token': user.get('session', {}).get('access_token')
                if isinstance(user.get('session'), dict)
                else getattr(user.get('session'), 'access_token', None),
