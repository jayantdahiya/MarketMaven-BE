...
from api.schemas.forecast import DailyForecastRequest, DailyForecastResponse, DailyForecastPoint
from api.models.prophet_forecaster import forecast_prophet, DataUnavailableError
...
class ForecastService:
    def __init__(self, config: dict | None = None, config_path: str | None = None):
        ...
        self.checkpoints_dir = Path(paths.get("checkpoints_dir", "artifacts/checkpoints/phase0"))
    ...
    def _load_model(self, model_name: str) -> torch.nn.Module | None:
        ...
        if "scaler_state" in state and state["scaler_state"]:
            self._scaler_cache[model_name] = FeatureScaler.from_state(state["scaler_state"])
        return model

    def _build_signal(self, predicted_return: float, threshold: float | None = None) -> tuple[str, float]:
    ...
    def _predict_prophet(self, req: DailyForecastRequest) -> DailyForecastResponse:
        try:
            out = forecast_prophet(req.asset_id, horizon_days=req.horizon_days, end_date=req.as_of_date)
        ...
        points = [
            ...
            for ts, trend in zip(out["timestamps"], out["trend"])
        ]
        return DailyForecastResponse(
            asset_id=req.asset_id,
            model="prophet",
