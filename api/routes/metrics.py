...
async def metrics_latest(request: Request, model: str | None = None) -> MetricsSummaryResponse:
    """Return latest evaluation metrics from reports dir (if any)."""
    cfg = getattr(request.app.state, "config", {})
    reports_dir = Path(cfg.get("paths", {}).get("reports_dir", "artifacts/reports/phase0"))
    if not reports_dir.exists():
        return MetricsSummaryResponse(
            mae=0.0, rmse=0.0, directional_accuracy=0.0, sharpe=0.0, sortino=0.0, max_drawdown=0.0, model=model, run_id=None
        )
