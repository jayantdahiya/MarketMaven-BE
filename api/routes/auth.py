...
from api.schemas.auth import SignUpRequest, LoginRequest, AuthResponse
...
async def signup(req: SignUpRequest, request: Request) -> AuthResponse:
    ...
    if service is None:
        raise HTTPException(status_code=503, detail="Auth service unavailable (missing Supabase)")
    out = service.signup(req.email, req.password)
    if out.get("access_token") is None and "error" in str(out.get("message", "")).lower():
