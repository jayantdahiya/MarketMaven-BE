...
class FeatureScaler:
    ...
    def __init__(self, mode: str = "per_asset_zscore", clip: float = 5.0):
        if mode not in ("per_asset_zscore", "global_zscore"):
            raise ValueError(f"mode must be per_asset_zscore or global_zscore, got {mode}")
        self.mode = mode
        self.clip = clip
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.mode == "per_asset_zscore":
            for aid, sc in self._scalers.items():
                ...
                if mask.any():
                    out.loc[mask, self._feature_cols] = sc.transform(df.loc[mask, self._feature_cols])
            # assets not seen in fit: leave as-is or use global fallback; spec says per-asset
        else:
            out[self._feature_cols] = self._global_scaler.transform(df[self._feature_cols])
    ...
    def get_state(self) -> dict[str, Any]:
        ...
        if self.mode == "per_asset_zscore":
            state["scalers"] = {aid: {"mean_": sc.mean_.tolist(), "scale_": sc.scale_.tolist()} for aid, sc in self._scalers.items()}
        else:
