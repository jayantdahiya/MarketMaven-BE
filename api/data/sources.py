...
def fetch_yfinance_data(
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if df is None or df.empty:
                    logger.warning(
                        'No data returned for %s (attempt %d), skipping',
                        symbol,
                        attempt + 1,
                    )
                    break
                ...
                if attempt == MAX_RETRIES - 1:
                    logger.warning(
                        'Failed to fetch %s after %d attempts: %s; skipping',
                        symbol,
                        MAX_RETRIES,
                        e,
                    )
    ...
    if (
        'timestamp' not in out.columns
        and out.index.name is None
        and hasattr(out.index, 'normalize')
    ):
