...
from api.data import features, sources, targets, splitters, normalization, datasets
...
class DataPipeline:
    ...
    def load_raw_data(self, data_path: str | None = None) -> pd.DataFrame:
        ...
        path = data_path or str(Path(self.cfg.get("paths", {}).get("data_dir", "artifacts/data/phase0")) / "raw_daily.parquet")
        if Path(path).exists():
    ...
    def split_by_date(self, df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        return splitters.split_by_date(df, train_end, val_end, test_end, date_col="timestamp")
    ...
    def fit_scaler(self, train_df: pd.DataFrame) -> normalization.FeatureScaler:
        ...
        feature_cols = self.data_cfg.get("feature_cols", ["close", "volume", "sma_15", "sma_45", "rsi_14", "macd", "macd_signal", "obv"])
    ...
    def build_dataloaders(
        ...
        train_ds = datasets.DailySequenceDataset(
            ...
            feature_cols, target_col, seq_len, horizon,
        )
        ...
        train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=shuffle_train, num_workers=0)
        val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=0)
        test_loader = DataLoader(test_ds, batch_size=batch_size, shuffle=False, num_workers=0)
        logger.info("DataLoaders: train=%d val=%d test=%d batches", len(train_loader), len(val_loader), len(test_loader))
    ...
    def build_inference_window(
        ...
        if scaler is None:
            raise RuntimeError("Scaler not fitted; run build_dataloaders or pass scaler")
        seq_len = self.data_cfg.get("seq_len", 60)
        ...
        if len(df) < seq_len:
            raise RuntimeError(f"After features, fewer than {seq_len} rows for {asset_id}")
