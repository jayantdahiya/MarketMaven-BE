...
from api.training import losses, checkpointing


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser()
    p.add_argument("--config", default="config/phase_0.yaml", help="Path to config YAML")
    p.add_argument("--seed", type=int, default=None, help="Single seed; if omitted use all from config")
    return p.parse_args()
...
def main() -> None:
    ...
    paths = cfg.get("paths", {})
    Path(paths.get("checkpoints_dir", "artifacts/checkpoints/phase0")).mkdir(parents=True, exist_ok=True)
    seeds = [args.seed] if args.seed is not None else cfg.get("training", {}).get("seeds", [42])
    ...
    for seed in seeds:
        ...
        scaler_path = str(Path(paths.get("data_dir", "artifacts/data/phase0")) / "scaler.joblib")
